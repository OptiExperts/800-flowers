...
jQuery(document).ready(function($) {
  ...
  function slideProductDetailBottom() {
   ...
   function loyaltiprogram(){
    $('.point-pop').on('click',function(){
      $('.cus-point-info').addClass('active');
      $('.overlay').addClass('active');
    });
    $('.overlay').on('click',function(){
      $('.cus-point-info').removeClass('active');
...
function MobileCartSlide() {
    $('.cart-detail .slick_product_new-h5').slick({
      infinite: true,
      slidesToShow: 4,
      slidesToScroll: 2,
      dots: true,
...
function desktopCartSlide() {
    $('.hide-on-mob .desktop .slick_product_new-h5').slick({
      infinite: true,
      slidesToShow: 6,
      slidesToScroll: 2,
      dots: false,
...
jQuery(document).ready(function($) {
  ...
  function jsmenu_fullwidth(){
    ...
    $('.js-full-width').css({'top' : top.top + height - 15 ,'position' : 'fixed' , 'left' :' 0' , 'right' : '0', 'width' : '100%' })

    $(window).scroll(function() {
      var height = $('.js_height_hd').outerHeight();
      if ($(window).scrollTop() >= height){      
        $('.js-full-width').css({'top' : height - 15 })
      }
      else{
        $('.js-full-width').css({'top' : top.top + height - 15  })
      }     
    });
  }
...
$('.free-shipping-info').removeClass('ajax-free-shipping');
	$(document).on('click','.enj-add-to-cart-btn',function(){

setTimeout(function(){
$('.free-shipping-info').addClass('ajax-free-shipping');
	  	var FreeShippingPrice = $('.free-shipping-info').attr('data-shipping');
	  	var progresstotal = $('.minicart .sub-total .total-price').html().replace("AED", " ");
        var progresstotal = progresstotal.replace(/[^0-9\.]+/g,"");
  ...
  document.querySelector(".progress-line").style.display = "none";
	...
	$(document).on('click','.produc_remove .fa-trash-o',function(){
setTimeout(function(){
	  	var FreeShippingPrice = $('.free-shipping-info').attr('data-shipping');
	  	var progresstotal = $('.minicart .sub-total .total-price').html().replace("AED", " ");
        var progresstotal = progresstotal.replace(/[^0-9\.]+/g,"");
...
$(document).on('click','.free-shipping-info .close-alert',function(){
console.log('test');
